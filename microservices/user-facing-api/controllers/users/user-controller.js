...
import uploadUserQuestions from "../userAnswers/uploadUserQuestions.js";
dotenv.config();
...
const createUser = async (req, res) => {
  try {
    ...
    // Call uploadUserQuestions after user creation
    try {
      await uploadUserQuestions({ params: { userId: user._id },body: { questionAnsObj:[] } }, res); // Call with mock req and res
    } catch (uploadError) {
      logger.log(LogLevel.ERROR, uploadError);
      return res.status(500).json({ message: "An error occurred while uploading user questions" });
    }
